...
const Search = () => {
    const { searchTerm, setSearchTerm, loading, data, error, handleSearch, showResults, setShowResults, clearError } = useUser()
    ...
    return (
        <div>
            <div id={darkTheme ? 'searchDark' : 'search'}>
                <form>
                    <label>
                        <div id='searchLeft'>
                            <svg id='searchIcon' xmlns="http://www.w3.org/2000/svg" width="25" height="24" viewBox="0 0 25 24" fill="none">
                            ...
                            <input id={darkTheme ? 'inputDark' : 'input'} type='text' placeholder='Search GitHub username…' value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} onKeyDown={handleKeyPress}/>
                        </div>
                        <div id='buttonResults'>
                            <button id='buttonSearch' type='button' onClick={() => {handleSearch(); setShowResults(true); clearError()}} disabled={loading}>
            ...
            {error && !showResults && <p id='error'>{error}</p>}
        </div>
    )
}
